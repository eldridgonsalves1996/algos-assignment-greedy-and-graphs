 ...
 * Does this compile or finish running within 5 seconds? Y/N, YES IT COMPILES 
 */
 ...
 * not mess with the existing formatting and indentation. You don't need to use
...
public class PhysicsExperiment {
  ...
  public int[][] scheduleExperiments(int numStudents, int numSteps, int[][] signUpTable) {
    ...
    // Your code goes here
    /*
    * "have a while loop to check what steps are possible to do and
    * there is a for loop outside that increments by how many steps that was done"
    */

    int step = 0; //we will begin the algorithm by checking for the first step of the physics experiment
    ArrayList<Integer> stepsScheduled = new ArrayList<Integer>(); //keep track of steps
    for(int i = 0; i <= numStudents; i++){ //loop through the students to see which student does what step
      for(int j = step; j < signUpTable[i].length; j++){ //loop through all the steps, accounting for which steps are done
        if(stepsScheduled.contains(j)){ //if the ArrayList contains step j, that means we already considered that step
          continue; //skip the iteration for step j because it's already in the ArrayList
        }
        if(step == numSteps && signUpTable[i][j] == 1){ //if we're at the last step and student i can do the current step j
          scheduleTable[i][j] = 1; //set scheduleTable[i][j] to be 1
          stepsScheduled.add(j); //add step j to the ArrayList
          return scheduleTable; //return the table because there are no more steps
        }
        else if(signUpTable[i][j] == 1){ //if student i can do the current step j
          scheduleTable[i][j] = 1; //set scheduleTable[i][j] to be 1
          stepsScheduled.add(j); //add step j to the ArrayList
